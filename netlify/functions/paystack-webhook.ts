...
export const handler = async (event: any) => {
  try {
    ...
    if (hash !== signature) {
      ...
      return {
        statusCode: 400,
        body: JSON.stringify({ status: false, message: "Invalid signature" }),
      };
    }
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ status: true, message: "Webhook processed" }),
    };
  } catch (error: any) {
...
async function handleSubscriptionNotRenew(data: any) {
  try {
    ...
    if (error) {
      ...
      console.log(
        `Successfully cancelled subscription for clinic ${metadata.clinic_id}`
      );
    }
  } catch (error) {
